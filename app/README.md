...
- **Google reCAPTCHA v2** — captcha do formulário
...
| Variável | Descrição |
| --- | --- |
| `NEXT_PUBLIC_RECAPTCHA_SITE_KEY` | Site key do Google reCAPTCHA v2 (widget) |
| `RECAPTCHA_SECRET_KEY` | Secret key do reCAPTCHA (verificação server-side) |
...
Para desenvolvimento local, use as chaves reais do reCAPTCHA (o domínio
`localhost` precisa estar autorizado no admin do Google). Sem `RESEND_API_KEY`
em desenvolvimento, as solicitações são registradas no console em vez de
enviadas por e-mail.

## Fluxo de orçamento

`POST /api/quote` processa na ordem: rate limit por IP (5/10min) → validação
zod → honeypot (sucesso falso para bots) → verificação reCAPTCHA → envio via
